...
const navItems = [
  ...
  {
    label: "Reporte por tester",
    href: "/reports/testers",
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
      </svg>
...
export function Sidebar() {
  ...
  const visibleItems = navItems
    .filter(item => {
      ...
      if (item.href === "/reports/client") return can("reports", "read");
      if (item.href === "/reports/testers") return can("reports", "read");
  ...
  return (
    ...
    >
      ...
      {!isClientPm && (can("roles", "read") || can("audit", "read") || can("holidays", "read")) && (
        <div className="px-3 pb-3">
          ...
          {can("holidays", "read") && (
            <Link
              ...
              className={`group relative flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-all duration-200 rounded-none ${
                pathname.startsWith("/settings/holidays")
                  ? "bg-white/5 text-white border-l-[3px] border-l-[#4A90D9]"
                  : "text-white/50 hover:text-white/80 hover:bg-white/[0.03] border-l-[3px] border-l-transparent"
              }`}
            >
