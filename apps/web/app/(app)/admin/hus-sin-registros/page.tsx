...
export default function HusSinRegistrosPage() {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    apiClient<Response>("/api/admin/hus-sin-registros")
      .then(setData)
    ...
    <div className="p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/70">
              HUs en Diseño o Ejecución sin ningún registro cargado (diseñados=0 y ejecutados=0). Salen vacías en informes/presentaciones.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-4">
        <div className="rounded-xl border bg-white px-5 py-3 shadow-sm">
          ...
          <p className="mt-0.5 font-mono text-2xl font-bold tabular-nums text-[#dc2626]">
            {data ? data.totalHus : "—"}
          </p>
        </div>
        <div className="rounded-xl border bg-white px-5 py-3 shadow-sm">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500">Proyectos afectados</p>
          <p className="mt-0.5 font-mono text-2xl font-bold tabular-nums text-[#2E5FA3]">
        ...
        <div className="ml-auto">
          <input
            ...
            placeholder="Buscar HU, ID, proyecto o analista…"
            className="w-72 rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-[#1F3864] focus:outline-none focus:ring-2 focus:ring-[#1F3864]/30"
          />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-emerald-200 bg-emerald-50/50 p-10 text-center text-sm text-emerald-700">
          ✓ No hay HUs en Diseño o Ejecución sin registros. Todo cargado.
        </div>
      ) : shownHus === 0 ? (
                ...
                <tbody className="divide-y divide-gray-50">
                  {p.hus.map((h) => (
                    <tr key={h.storyId} className="hover:bg-gray-50/60">
                      ...
                      <td className="px-3 py-2.5"><StatusChip label={h.statusLabel} /></td>
