...
router.put("/:id", requirePermission("cycles", "update") as any, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const cycle = await prisma.$transaction(async (tx) => {
      ...
      // Cascada idempotente: SIEMPRE intenta sincronizar assignments con las
      // fechas actuales del ciclo, aunque éste no haya cambiado en este PUT.
      // Esto cubre el caso "edité antes del fix de cascada y quedaron
      // desfasados" — ahora basta con re-abrir el modal y guardar para sync.
      // Solo extiende, nunca acorta (preserva data). Excluye PRODUCTION/ON_HOLD.
      const cycleStart = updated.startDate;
      const cycleEnd = updated.endDate;
      if (cycleStart || cycleEnd) {
        ...
        for (const a of openAssignments) {
          ...
          if (cycleStart && a.startDate > cycleStart) {
            aDiffs.push({ field: "startDate", oldValue: a.startDate, newValue: cycleStart });
            aData.startDate = cycleStart;
          }
          if (cycleEnd && (a.endDate === null || a.endDate < cycleEnd)) {
            aDiffs.push({ field: "endDate", oldValue: a.endDate, newValue: cycleEnd });
            aData.endDate = cycleEnd;
          }
          if (aDiffs.length > 0) {
            await tx.testerAssignment.update({ where: { id: a.id }, data: aData });
            const propagationReason = data.reason
              ? `Propagado desde ciclo: ${data.reason}`
              : "Sincronización con fechas actuales del ciclo";
            await writeDateChangeLogs(tx, {
